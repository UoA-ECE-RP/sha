...
# intial location of the ball is (5,5)
ode0x = Ode(sympify("diff(x(t))"), sympify("x(t)"), 5)
ode0y = Ode(sympify("diff(y(t))"), sympify("y(t)"), 5)
ode1x = Ode(sympify("diff(x(t))-0.866"), sympify("x(t)"), 0)
# initalisation to zero does not make sense
ode1y = Ode(sympify("diff(y(t))-0.500"), sympify("y(t)"), 0)
