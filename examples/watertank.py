...
t1 = Loc("t1", [ode1], [], Guard(sympify("x>=20 & x<=100")))
t2 = Loc("t2", [ode2], [], Guard(sympify("x>=100 & x<=100")))
t3 = Loc("t3", [ode3], [], Guard(sympify("x>=20 & x<=100")))
t4 = Loc("t4", [ode4], [], Guard(sympify("x>=20 & x<=20")))

# The edges
e1 = Edge(t1, t2, Guard(sympify("x>=100 & x<=100")),
          [Update.Update2(Symbol('x'), Symbol('x'))],
          [Event("B")])
e2 = Edge(t2, t3, Guard(sympify("True")),
          [Update.Update2(Symbol('x'), Symbol('x'))],
          [Event("OFF")])
e3 = Edge(t1, t3, Guard(sympify("True")),
          [Update.Update2(Symbol('x'), Symbol('x'))],
          [Event("OFF")])
e4 = Edge(t3, t1, Guard(sympify("True")),
          [Update.Update2(Symbol('x'), Symbol('x'))],
          [Event("ON")])
