...
#define OFILEP "plantout.csv"
#define IFILEP "plantin.csv"
...
unsigned char add1, add2, remove1, remove2;
...
void readInputPlant() {
  ...
  */
  ...
  fprintf(stdout, "%s:%d %s:%d %s:%d %s:%d tick:%ld\n", "add1", add1,
    "add2", add2, "remove1", remove1, "remove2", remove2, tick);
  fflush(stdout);
}

/* Write output x to file */
void writeOutputPlant(){
  static unsigned char count = 0;
  if (0 == count){
    fo = fopen(OFILEP, "w");
    if (fo == NULL){
