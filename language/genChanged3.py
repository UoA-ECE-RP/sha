...
def OdeCodegen(os, name, han):
    ...
    for i in xrange(0, len(os)):
        with patterns:
            ...
            odes[i] = ode
            diffx[i] = S("diff(" +  str(var) + ")")
            vars[i] = var
            #sx[i] = Symbol('var.func')
            sx[i] = S(str(var.func))
            iValues[i] = iValue
    try:
...
def getInvariantAndOdeExpr(han, loc, events, tab, contVars,
                           funcrs, ifuncrs):
    with patterns:
        ...
        for i, od in enumerate(odes):
            with patterns:
                ...
                stmts += [tab+ han + '_' + str(var.func)+' = '+rr+';']
                    ...
                    if not o.rhs.is_Number:
                        ...
                        if sign(s).is_Number:
                            if mm != []:
                                if sign(s) > 0:
                                    cb = str(max(mm))
                                    stmts += ['if('+ han + '_' + str(var.func)+'_u > ' +
                                              cb + ' && ' + han + '_' +
                                              str(var.func) + '_init' +
                                              ' <= ' + cb + ')']
                                    stmts += [tab + han + '_' + str(
                            ...
                            if mm != []:
                                cb = str(max(mm))
                                stmts += ['if(' + han + '_' + str(var.func)+'_u > ' +
                                          cb + ' && '+ han + '_' + str(var.func)+'_init' +
                                          ' <= ' + cb + ')']
                                stmts += [tab + han + '_' + str(
                                    var.func) + '_u = ' + cb + ';']
                                # Decreasing or non-increasing function
                                cb = str(min(mm))
                                stmts += ['if(' + han + '_' + str(var.func) +
                                          '_u < ' + cb + ' && ' + han + '_' +
...
def getEAndGAndU(han, edge, events):
    with patterns:
...
def makeReactionFunction(fname, locs, edges, snames, events,
    ...
    for i, state in enumerate(snames):
        ...
        print i
        (invExpr, odeStmts,
         cstmts, cks) = getInvariantAndOdeExpr(fname, locs[i],
                                               events, tab,
                                               contVars,
                                               funcrs[i], ifuncrs[i])
        cks = list(cks) + ['fk']
        ...
        for cst in cstmts:
            ret += [tab*level+cst]
        #ve = fname + '_' + invExpr
        #print "haiasoiaosiojdosidjaksldj"
        #print ve
        ...
        for edge in edgesWithstateSource(edges, state):
            (eeExpr, egExpr, uStmts, dState) = getEAndGAndU(fname, edge,
                                                            events)
            #negExpr = fname + '_' + egExpr

            print "egExpr: ahahahahahahah"
            print egExpr
            print eeExpr
    ...
    updates = map(lambda x: tab * level + fname + '_' + x + ' = ' + fname + '_' + x +
                      '_u;', contVars)

    print "laaaaaaaaaaaA"
    print type(contVars)
    print contVars
    print type(contVars[0])
    ret += updates
...
def codeGen(ha):
    with patterns:
        ...
        #for i in xrange(len(ha)):
         #   Ha(han, ls, sloc, edges, gvs, igvs, iv, eiv, eov, eie, eoe) << ha[i]
        Ha(han, ls, sloc, edges, gvs, igvs, iv, eiv, eov, eie, eoe) << ha
        lnames = [None]*len(ls)
        ...
        with patterns:
            ...
            for i, o in enumerate(ol):
                with patterns:
                    ...
                    contSet = set([str(item) for sl in iv for item in sl])
        ...
        uContSet = set([str(item)+"_u" for sl in iv for item in sl])
        #uContDecl = map(lambda x: 'static double ' + x + ', '.join(uContSet) + ';', han)
        uContDecl = ['static double ' + han + '_' + ', '.join(uContSet) + ';']
        
        # Declare the init continous variables
        iContSet = set([str(item)+"_init" for sl in iv for item in sl])
        #iContDecl = map ('static double ' + x + ', '.join(iContSet) + ';', han)
        iContDecl = ['static double ' + han + '_' + ', '.join(iContSet) + ';']
        ...
        # The updated (init) continous variables
...
def getSha(ha):
    with patterns:
        ...
        return Ha(n, map(partial(updateLocNsteps, gvs), ls),
                  sloc, edges, gvs, igvs, iv, eiv, eov, eie, eoe)
...
def isWha(ha):
    with switch(ha):
        ...
        if Ha(han, ls, sloc, edges, gvs, igvs, iv, eiv, eov, eie, eoe):
...
def preprocess(ha):
    with switch(ha):
        ...
        if Ha(han, ls, sloc, edges, gvs, igvs, iv, eiv, eov, eie, eoe):
            ...
            return Ha(han, ls, sloc, edges, gvs, igvs, iv, eiv, eov, eie, eoe) << ha
        else:
